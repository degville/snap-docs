...
Snaps have the least possible privileges by default. Additional system resources are permitted through the **[interfaces](/explanation/interfaces/all-about-interfaces)** mechanism which changes the security profile for each snap.
...
The state of snapd, and many of its key functions, can be accessed through its <a href="../api/redoc-static.html">REST API</a>, enabling remote device configuration, updates and orchestration. 
