...
This interface is a companion interface to the {ref}`audio-playback <interfaces-audio-playback-interface>` interface, and is not intended to be used without it.
...
The design of this interface is based on the principle that the slot implementation of the audio service, such as {ref}`PulseAudio <interfaces-pulseaudio-interface>`, queries whether its audio-record slot is connected, leaving the audio service to mediate recording if it is.
