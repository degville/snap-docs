...
Audio recording is enabled with the companion {ref}`audio-record <interfaces-audio-record-interface>` interface, and unlike `audio-playback`, is not enabled (auto-connected) by default.
