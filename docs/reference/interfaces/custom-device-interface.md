...
Using the custom-device requires {ref}`Store approval and permissions <interfaces-process-for-aliases-auto-connections-and-tracks>`, both to allow the presence of the slot, and to set up elements such as the slot self-connecting exactly to the plug on the app. The more specific the tagging information provided by the slot, the easier it will be to allow for this.
