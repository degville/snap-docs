...
See {ref}`How components work <explanation-how-snaps-work-snap-components>` for further details.
...
It is also possible to install components from local files. If {ref}`unasserted <explanation-security-assertions>`, they can be installed only if the host snap is unasserted too and you will need to use the --dangerous flag:
