...
Refer to [Managing updates](/how-to-guides/manage-snaps/manage-updates) for more details.
...
User data, such as data generated by the snap and stored in a database, is often stored in a _common_ directory and will not be reverted. See [Data locations](/reference/administration/data-locations) for more details on what information is stored and where.
...
For more details on where snaps store their data, see [Data locations](/reference/administration/data-locations).

## Create and restore a snapshot

A *snapshot* is a copy of the user, system and configuration data stored by *snapd* for one or more snaps on your system, and a snapshot of the data found in both `$HOME/snap/<snap-name>` and `/var/snap/<snap-name>` is stored in `/var/lib/snapd/snapshots/` (see [Data locations](/reference/administration/data-locations) for more details).
