...
Without [custom flags at installation](https://snapcraft.io/docs/install-modes), or subsequent [interface connections](/how-to-guides/manage-snaps/connect-interfaces), snaps remain confined to a restrictive security sandbox, preventing access to system resources outside the snap.
...
Each command declared with the `apps` [snap metadata](/reference/development/yaml-schemas/the-snap-format) is tracked by the system assigning a security label to the command.
...
See [AppArmor violations](/how-to-guides/snap-development/debug-snaps) for help tracking AppArmor problems.
...
See [Seccomp violations](/explanation/security/security-policies) for help tracking Seccomp problems.
...
An interface may either auto-connect upon install, or require the user to manually connect them. Interface connections and disconnections are performed via the  `snap connect`  and  `snap disconnect` commands. See [interfaces](/explanation/interfaces/all-about-interfaces) for details.
...
By default, a service running from a snap needs to be restarted whenever the snap is refreshed (see [Services and daemons](/explanation/security/security-policies)) for more details).

Stopping and starting a service is a requirement to support [snap revert](/explanation/security/security-policies)) and its copying of a snap’s system data from the current version to the new version.

System data typically includes databases, data files, and configuration files (see [Data locations](/reference/administration/data-locations)), although all of this is up to the snap developer.
...
* [Snap specific configurations options](/how-to-guides/manage-snaps/configure-snaps)
* [Snapshots](/how-to-guides/manage-snaps/create-data-snapshots) of snap [user data](/reference/administration/data-locations)
...
* [Persisted data on Ubuntu Core devices](/reference/administration/data-locations)
...
* [How to identify which interfaces are required to solve policy violations](/how-to-guides/snap-development/debug-snaps)
  * [AppArmor violations](/how-to-guides/snap-development/debug-snaps)
  * [Seccomp violations](/how-to-guides/snap-development/debug-snaps)
  * [File permissions and cgroups](/explanation/security/security-policies)
