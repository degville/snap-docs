...
The creator of a snap selects the interfaces that a snap requires in order to function correctly. Common interfaces include those that provide access to the {ref}`network <interfaces-network-interface>`, {ref}`desktop features <interfaces-desktop-interface>` and {ref}`audio recording <interfaces-audio-record-interface>`.
...
Connections are made either automatically at install time or manually, depending on their function. The desktop interface is connected automatically, for instance, whereas the camera interface is not. The *Auto-connect* column in the {ref}`Supported interfaces <ref-index_interfaces>` table for lists  whether an interface automatically connects or not. See the {ref}`Interface Auto-connection mechanism <explanation-interfaces-interface-auto-connection>` for implementation details.

As with {ref}`classic confinement <explanation-security-classic-confinement>`, a snap's publisher can request an *assertion* to automatically connect an otherwise non-auto-connecting interface.
...
See {ref}`Interface management <how-to-guides-work-with-snaps-connect-interfaces>` for further interface details, including how to disconnect interfaces and make manual connections, and {ref}`Security policy and sandboxing <explanation-security-security-policies>` for more information on how confinement is implemented.
