...
Hardware IO (input/output) interfaces, including the {ref}`serial-port <interfaces-serial-port-interface>`, {ref}`gpio <interfaces-gpio-interface>`, {ref}`gpio-chardev <interfaces-gpio-chardev>` and {ref}`i2c <interfaces-i2c-interface>` interfaces, are designed to be used on devices running {ref}`Ubuntu Core <ref-glossary_ubuntu-core>`. These interfaces are exposed as _slots_ from a device's {ref}`gadget snap <reference-development-yaml-schemas-the-gadget-snap>` which is used to define and configure a device's system properties.
...
- {ref}`gpio-control interface <interfaces-gpio-control-interface>`
- {ref}`serial-port interface <interfaces-serial-port-interface>` (experimental support)

See {ref}`Supported interfaces <ref-index_interfaces>` for a complete list of interfaces.
...
1. **Connection permissions**: {ref}`auto-connect <explanation-interfaces-interface-auto-connection>` | {ref}`privileged <reference-operations-interfaces-super-privileged-interfaces>` | {ref}`super-privileged <reference-operations-interfaces-super-privileged-interfaces>`
   </br>Connection requirements are dependent on which store a developer is using.
     - [Global Snap Store](https://forum.snapcraft.io/t/glossary/14612#heading--snap-store): privileged and super-privileged interfaces require store approval because of the level of trust and permissiveness these interfaces have, which is also why certain interfaces need certain oversight. See {ref}`Permission requests <interfaces-process-for-aliases-auto-connections-and-tracks>` for further details.
...
1. **Interface connections**: hardware IO interfaces | app-provided interfaces | other interfaces
    * **Hardware IO interfaces**: These require either a {ref}`slot <how-to-guides-work-with-snaps-connect-interfaces>` to be defined by a device's _gadget snap_ or an interface with {ref}`Hotplug support <interfaces-hotplug-support>`, in which case the slot appears from the system snap.
      * An unconstrained {ref}`auto-connection <explanation-interfaces-interface-auto-connection>` cannot be used because there may be _many slots of a given interface_, resulting in ambiguity that requires  an extensive set of store rules to manage and maintain.
      * Each plug should therefore be connected to a slot, for example:
      ...
      - This kind of 1-to-1 connections can usually be established via {ref}`slot rules in the snap-declaration <explanation-interfaces-interface-auto-connection>` for the gadget.
    * **App-provided interfaces**: slots are defined by apps, or occasionally from the gadget snap,
      * May require access, such as from the {ref}`content <interfaces-content-interface>` or {ref}`shared-memory <interfaces-shared-memory-interface>` interfaces.
      * A slot might may be provided by the system snap to cover the case of an equivalent system service, such as {ref}`audio-playback <interfaces-audio-playback-interface>`
      * the slot might be {ref}`super-privileged <reference-operations-interfaces-super-privileged-interfaces>`
