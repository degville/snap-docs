...
Timing output from a snap can be a useful measure of overall performance, but it can also reveal performance bottlenecks and targets for improvement. The [snap debug](/how-to-guides/snap-development/debug-snaps) command includes several options to produce timing output for execution time, internal _ensure_ activities and subsystem startup times.
